...
There is a simple intuition of what it means to be a friend. A friend of Augustus cares about Augustus more than about other people. And, if we see Augustus, then we infer to see friend(s) of Augustus also. Please refer to [our arXiv preprint](https://arxiv.org/abs/2512.24843) for a more formal introduction.


Let's picture a set of expression pattern, each represents a biological process. Technically, each pattern is a vector of expression loads of the involved genes.
...
The main idea of the **friends.test**  test is that if a row $t_i$ is not preferred by any column(s), the distribution of the ranks of the row in all the columns $r(t_i,c_j)$ behaves like $j$ i.i.d integers that are uniformly distributed in $1..n$. 
...
So, flower #14 is in top 50% in Sepal.Width and in lowest 5% for all other sizes. It is exactly what we describe as "Sepal.Width is friend for flower #14 and the flower #14 is a marker for Sepal.Width".
...
CoGAPS decomposes an expression dataset into patterns of expression, ideally, each of them represent an underlying biological process. We use the [PDAC dataset](https://zenodo.org/records/7709664) from the [Johnson _et al_, 2023](https://www.nature.com/articles/s41596-023-00892-x) as an example. 
...
We use here the default version of `friends.test` that selects markers by the KS test. As far as there is ~15000 genes, we use Bonferroni p-value adjustment rather than the default "BH". 
...
The leaf is a 3-integer vector: the marker index and the friend index in the matrix we started with, and the rank of the friend for the marker. Let's look at the first element; its rank is 1 as it is the first.
...
The format is flexible enough, we exemplify possible transformations [later](#the-result-format-transformations). The format is common for the `friends.test` and `friends.test.bic` main functions.
...
The model splitted the patterns into two sets: Pattern_1 where the marker has a very low rank, and the remainder, as expected.
...
That is what the friends.test is a unique test to detect: the `r names(best_friends_of_genes)` has a moderate rank in `r names(best_friends_of_genes[[1]])`, but it is the best of its ranks in all other patterns and it is significantly higher than all of them.


## Gene set enrichment

To perform an example of gene set enrichment analysis, we use the Bayesian version of the test, `friends.test.bic`. We restrict the number of friends for a marker by 4, a half of number of the patterns, to keep them all in the similar "direction". The required parameter `prior.to.have.friends` is set to approximately 1/(number of genes). We run 2 parallel processes, see [Parallel calculation](#parallel-calculation).
...
For the gene overrepresentation analysis, we need to generate a list of markers (genes) for each pattern. So, we swap the levels of the list, and we do not need leaves. Here, we can reverse the list via a data frame. It works if the number of elements do not reach hundred of thousands. 
...
As far as we have gene lists, let's examine them for overrepresentation in the gene collections from [MSigDB](https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp). We use `fgsea` and `msigdbr` for that.
...
The result of both `friends.test` and `friends.test.bic` functions depends on random state generator. The dependency is because the test starts with ranking of the elements of the input matrix in the columns. The test requires the rank values to be integer, so the ties are resolved randomly. It you want a reproducible result, keep the initial random generator state. Another option is to rank the matrix before the test call and then to pass the rank matrix as the input. It does not consume a lot of additional time, and the ranking of already-ranked does not depend on random seeds. The special case when this approach is the only is if some additional information is used in the ranking.

## Parallel calculation

Both main functions, `friends.test` and `friends.test.bic`, can distribute the row-wise work through `BiocParallel`. Pass `BPPARAM = BiocParallel::MulticoreParam(workers = n)` on Unix-like systems or `BPPARAM = BiocParallel::SnowParam(workers = n)` on all platforms. If you also set `.progress = TRUE`, the selected `BiocParallel` backend shows its text progress bar.

Here is a minimal parallel example using a socket backend that also works on Windows:
...
Suvorikova A, Kroshnin A, Lvovs D, Mukhina V, Mironov A, Fertig EJ, Danilova L, Favorov A (2025).
*friends.test: identifying specificity by detecting structural breaks in entity interactions.*
